...
const KEYFRAMES = `
  ...
  @keyframes orbFloat  { 0%,100%{transform:translate(0,0) scale(1)} 33%{transform:translate(20px,-15px) scale(1.04)} 66%{transform:translate(-12px,10px) scale(0.97)} }
  ...
  @keyframes slideUpFade{ from{opacity:0;transform:translateY(30px) scale(0.96)} to{opacity:1;transform:translateY(0) scale(1)} }
...
function CinematicBackground({ mouseX, mouseY }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Responsive grid that shifts slightly with mouse
      const mx = (mouse.current.x / canvas.width - 0.5) * 6;
      const my = (mouse.current.y / canvas.height - 0.5) * 6;
      const gap = 48;
      ctx.strokeStyle = "rgba(255,255,255,0.022)";
      ctx.lineWidth = 0.5;
      for (let x = mx % gap; x < canvas.width; x += gap) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, canvas.height);
        ctx.stroke();
      }
      for (let y = my % gap; y < canvas.height; y += gap) {
        ctx.beginPath();
        ctx.moveTo(0, y);
      ...
      if (spotX > 0 && spotY > 0) {
        ...
        spot.addColorStop(0, "rgba(230,57,70,0.04)");
        spot.addColorStop(1, "rgba(230,57,70,0)");
        ctx.fillStyle = spot;
...
// ── Ripple button ──────────────────────────────────────────────────
function RippleButton({ onClick, disabled, children, style }) {
...
export default function LoginPage() {
  ...
  const [phase, setPhase] = useState(0); // 0=hidden 1=panels 2=content 3=form
        ...
        >
          ...
          <div
            style={{
              ...
              filter: "blur(20px)",
              pointerEvents: "none",
              animation: "orbFloat 10s ease-in-out 1.5s infinite",
            }}
          />
          ...
          >
            ...
            <div
              style={{
                ...
                marginBottom: "1.5rem",
                opacity: phase >= 2 ? 1 : 0,
                transform: phase >= 2 ? "none" : "translateX(-20px)",
              ...
              >
                ...
                >
                  <div
                    style={{
                      animation:
                        phase >= 3 ? `fadeUp 0.55s ${ease} 0.05s both` : "none",
                    }}
                  >
                    <FloatingInput
                      id="email"
                  ...
                  <div
                    style={{
                      animation:
                        phase >= 3 ? `fadeUp 0.55s ${ease} 0.15s both` : "none",
                    }}
                  >
                    <FloatingInput
                      id="password"
                ...
                <div
                  style={{
                    animation:
                      phase >= 3 ? `fadeUp 0.55s ${ease} 0.25s both` : "none",
                    marginTop: "1.5rem",
                  }}
                >
                  <RippleButton
                    ...
                    style={{
                      ...
                      transition:
                        "background 0.35s ease, box-shadow 0.25s ease, transform 0.12s ease",
                      border: "none",
              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.background = "rgba(230,57,70,0.09)";
                      e.currentTarget.style.transform = "scale(1)";
                    }}
                  >
                    Auto-fill ↗
